...
class DepartmentMapper(graphene.ObjectType):
    ...
    def resolve_director(self, info):
    	...
    	return PersonMapper.init_scalar(mongo_mediator.get_person_by_id(self.director))
    ...
    @staticmethod
    def eject(id, name, vk, director):
        return [DepartmentMapper.init_scalar(item) for item in mongo_mediator.select_departments(name = name, vk = vk, director = director, ids = {'_id': id})]

    @staticmethod
    def init_scalar(item):
        return DepartmentMapper(id = str(item['_id']), name = item['name'], vk = item.get('vk'), director = item.get('director'))
